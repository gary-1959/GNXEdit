...
def printpacked_lin(packed, compareto, comment):
    ...
    if comment == None:
        comment = "No commentgate"

    # show patch parameters only
    if packed[6] != 0x2A:   # code 2A only
        return
    if packed[11] != 0x3D or packed[12] != 0x07:  # subcode 3C 06
        return
    
    unpacked = []
    print("MNFR ID: {:02X} DEVICE ID: {:02X} COMMAND: {:02X} SUBCODE: {:02X} {:02X}".format(packed[3], packed[5], packed[6], packed[11], packed[12]) )
    pint = packed[7:-1]
    block = 0
    mask = 0b10000000
    n = 0
